...
async function Page() {
  const { landingHyperText, openTime, closeTime, backgroundImageUrl } =
    await getSetting();
  ...
  return (
    <main className="relative flex flex-col items-center w-screen pt-5 overflow-hidden sm:pt-32 scroll-smooth md:pt-24">
      <div className="flex justify-center w-full px-2 mx-auto text-xl font-bold text-right text-white md:z-0 sm:max-w-2xl lg:max-w-4xl xl:max-w-6xl md:pt-10 sm:text-2xl md:text-3xl lg:text-4xl lg:px-4">
        <p>{landingHyperText}</p>
      </div>
      <HeroAnimation backgroundImageUrl={backgroundImageUrl} />
